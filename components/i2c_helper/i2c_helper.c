...
esp_err_t i2c_helper_write_reg(uint8_t slave_id, uint8_t reg_addr, uint8_t *data_wr, size_t size)
{
    ...
    esp_err_t ret = i2c_master_cmd_begin(I2C_MASTER_NUM, cmd, portTICK_PERIOD_MS); //1000 / portTICK_RATE_MS);
...
esp_err_t i2c_helper_read_reg(uint8_t slave_id, uint8_t reg_addr, uint8_t *data_rd, size_t size)
{
    ...
    ret = i2c_master_cmd_begin(I2C_MASTER_NUM, cmd, portTICK_PERIOD_MS); //1000 / portTICK_RATE_MS);1000 / portTICK_RATE_MS);
    i2c_cmd_link_delete(cmd);
    return ret;
}
